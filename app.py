...
def dashboard(
    ...
    <body>
        ...
        <div class="card"><h2>Asset Mode</h2>
            <p><b>Current Asset Mode:</b> {state.get('asset_mode', 'stocks')}</p>
            <form action="/update-asset-mode" method="post">
                <label><input type="radio" name="asset_mode" value="stocks" {'checked' if state.get('asset_mode', 'stocks') == 'stocks' else ''}> Stocks Only</label><br>
                <label><input type="radio" name="asset_mode" value="crypto" {'checked' if state.get('asset_mode', 'stocks') == 'crypto' else ''}> Crypto Only</label><br>
                <label><input type="radio" name="asset_mode" value="both" {'checked' if state.get('asset_mode', 'stocks') == 'both' else ''}> Both Stocks + Crypto</label><br>
                <button type="submit">Save Asset Mode</button>
            </form>
            <p><small>Stocks obey market hours. Crypto symbols like BTC/USD and ETH/USD can trade 24/7 when allowed.</small></p>
        </div>
